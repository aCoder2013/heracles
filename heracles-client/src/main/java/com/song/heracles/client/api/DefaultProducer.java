...
public class DefaultProducer implements Producer {
    ...
    public DefaultProducer(ProducerConfiguration producerConfiguration,
        HeraclesApiGrpc.HeraclesApiVertxStub heraclesClient, long producerId) {
    ...
    public void start() throws InterruptedException, HeraclesClientException {
        ...
        HeraclesProto.ProducerConnectRequest producerConnectRequest = HeraclesProto.ProducerConnectRequest
            .newBuilder()
    ...
    public CompletableFuture<MessageId> sendAsync(byte[] message) {
        ...
        HeraclesProto.SendMessageRequest sendMessageRequest = HeraclesProto
            ...
            .setProducerId(producerId)
            .setMessage(
                HeraclesProto.Message.newBuilder().setBody(ByteString.copyFrom(message)).build())
            .build();
        heraclesClient.handleSendMessage(sendMessageRequest, asyncResult -> {
            if (asyncResult.succeeded()) {
                ...
                sendAsyncFuture.complete(
                    new MessageId(messageId.getLogSegmentSequenceNo(), messageId.getEntryId(),
                        messageId.getSlotId()));
            } else {
