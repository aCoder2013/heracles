...
public class DefaultStream implements Stream {
    ...
    public DefaultStream(String streamName, String owner, DistributedLogConfiguration dlConfig,
    ...
    private CompletableFuture<AsyncLogWriter> acquireStream() {
        ...
        this.distributedLogManager.openAsyncLogWriter()
            .whenCompleteAsync((asyncLogWriter, throwable) -> {
                if (throwable != null) {
    ...
    private void processAcquireStreamSuccess(CompletableFuture<AsyncLogWriter> completableFuture,
        AsyncLogWriter asyncLogWriter) {
        completableFuture.complete(asyncLogWriter);
    }

    private void processAcquireStreamFailed(CompletableFuture<AsyncLogWriter> completableFuture,
        Throwable throwable) {
        if (throwable instanceof AlreadyClosedException) {
            log.error("Encountered unexpected exception when writing data into stream : ",
                this.streamName, throwable);
        } else {
            ...
            try {
                ...
                if (asyncLogReader != null) {
                    ...
                    completableFuture.completeExceptionally(
                        new StreamUnavailableException("Get AsyncLogReader from DLog failed."));
                }
            } catch (Exception e) {
                completableFuture.completeExceptionally(e);
            }
    ...
    public void submitOp(StreamOp streamOp) {
        if (StreamState.isUnavailable(streamState)) {
            streamOp.fail(new StreamNotAvailableStorageException(
            ...
            synchronized (this) {
                if (StreamState.isUnavailable(streamState)) {
                    ...
                    streamOp.fail(new StreamNotReadyStorageException(
                        "Stream " + streamName + " is not ready yet."));
                }
            }
        }
    }
    ...
    private synchronized AsyncLogWriter setStreamStatus(StreamState newState,
        ...
        if (oldState != this.streamState) {
            log.info(
                "Stream {} status already changed from {} -> {} when trying to change it to {}",
        ...
        if (null != owner && owner.equals(clientId)) {
            log.error(
                "I am waiting myself {} to release lock on stream {}, so have to shut myself down :",
                owner, this.streamName, t);
    ...
    private void executeOperation(final StreamOp streamOp) {
        ...
        if (writer != null) {
                ...
                streamOp.execute(writer, idGenerator)
                    .exceptionally(throwable -> {
                        if (throwable instanceof HeraclesStorageException) {
                            ...
                            log.error(
                                "Failed to execute streamOp with code :" + heraclesStorageException
                                    .getCode(), heraclesStorageException);
            ...
            if (lastException != null) {
                ...
                streamOp.fail(new StreamNotAvailableStorageException(
                    "Stream " + streamName + " is unavailable."));
            }
        }
    }
    ...
    public enum StreamState {
        ...
        public static boolean isUnavailable(StreamState status) {
            return StreamState.ERROR == status || StreamState.CLOSING == status
                || StreamState.CLOSED == status;
        }
    }
}
