...
public class WriteOp extends AbstractStreamOp<WriteResponse> {
    ...
    @Override
    public CompletableFuture<WriteResponse> doExecute(AsyncLogWriter asyncLogWriter,
        ...
        if (!streamName.equals(asyncLogWriter.getStreamName())) {
            log.error("Stream name mismatch : {}, writer : {}.", streamName,
                asyncLogWriter.getStreamName());
            WriteResponse response = new WriteResponse();
            response.setCode(ErrorCode.ILLEGAL_STATE);
            response.setMessage(
                "Stream name mismatch : " + streamName + ",dlog writer name :" + asyncLogWriter
                    .getStreamName());
