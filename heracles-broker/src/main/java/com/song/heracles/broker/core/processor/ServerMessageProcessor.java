...
public class ServerMessageProcessor extends HeraclesApiGrpc.HeraclesApiVertxImplBase {
  ...
  public void handleProducerConnect(HeraclesProto.ProducerConnectRequest request,
    ...
    if (existingProducerFuture != null) {
      if (existingProducerFuture.isDone() && !existingProducerFuture.isCompletedExceptionally()) {
        ...
        ErrorCode errorCode = !existingProducerFuture.isDone() ? ErrorCode.STREAM_NOT_READY :
      ...
      producerManager.create(topic)
          ...
          .exceptionally(throwable -> {
            ...
            response.fail(new HeraclesException("Create producer failed:" + throwable.getMessage(),
  ...
  public void handleConsumerConnect(ConsumerConnectRequest request,
    ...
    CompletableFuture<Consumer> previousFuture = consumers.putIfAbsent(consumerId, producerFuture);
    if (previousFuture != null) {
      if (previousFuture.isDone() && !previousFuture.isCompletedExceptionally()) {
        ...
        ErrorCode errorCode =
            previousFuture.isDone() ? getErrorCode(previousFuture) : ErrorCode.STREAM_NOT_READY;
        response.fail(
            new HeraclesException("Consumer is already on the connection", errorCode.getCode()));
      }
    } else {
      ...
      consumerManager.create(topic).thenAccept(consumer -> {
        ...
        response.fail(new HeraclesException("Create consumer failed:" + throwable.getMessage(),
            ErrorCode.UNKNOWN_ERROR.getCode()));
        producerFuture.completeExceptionally(throwable);
  ...
  public void handlePullOffset(PullOffsetRequest request, Future<PullOffsetResponse> response) {
    ...
    try {
      ...
      response.fail(new HeraclesException(
          "Read offset for :" + consumer.getPartitionedTopic().getOriginalTopic() + " failed.",
