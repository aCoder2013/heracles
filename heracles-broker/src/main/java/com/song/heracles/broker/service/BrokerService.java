        ...
        if (state.compareAndSet(State.UNINITIALIZED, State.INITIALIZING)) {
            ...
            try {
                ...
                vertxServer = VertxServerBuilder
                    .forAddress(vertx, StringUtils
                        .defaultIfBlank(brokerConfiguration.getBindAddress(),
                            NetUtils.getLocalhost()), brokerConfiguration.getBrokerPort())
                    .addService(new ServerMessageProcessor(this))
                    .build();
                remotingServer = new RemotingServer(vertxServer);
                remotingServer.start();
                curatorFramework = CuratorFrameworkFactory.newClient("127.0.0.1:2181",
                    brokerConfiguration.getZooKeeperSessionTimeoutMillis(),
                    brokerConfiguration.getZookeeperConnectionTimeout(), new RetryNTimes(3, 3000));
                curatorFramework.start();
                producerNameGenerator = new ZkDistributedIdGenerator(curatorFramework,
                    PRODUCER_NAME_PATTERN, brokerConfiguration.getClusterName());
                consumerNameGenerator = new ZkDistributedIdGenerator(curatorFramework,
                    CONSUMER_NAME_PATTERN, brokerConfiguration.getClientId());
                ...
                Namespace namespace = NamespaceBuilder.newBuilder()
                    ...
                    .build();
                streamFactory = new DefaultStreamFactory(brokerConfiguration.getClientId(),
                    dLogConfig, namespace, orderedExecutor);
            ...
            throw new HeraclesException(
                "Can't start broker service ,current state is :" + state.get(),
