...
public class ZkOffsetStorage implements OffsetStorage {
			...
			synchronized (this) {
				...
				try {
					...
					if (StringUtils.isBlank(json)) {
						throw new IllegalArgumentException("Can't convert Offset to JSON format :" + offset);
					}
					this.curatorFramework.create()
						.creatingParentsIfNeeded()
						.withMode(CreateMode.PERSISTENT)
	...
	public void persistOffset(PartitionedTopic partitionedTopic) throws Exception {
		synchronized (this) {
			if (this.offsetCache.containsKey(partitionedTopic)) {
				...
				String json = GsonUtils.toJson(offset);
				if (StringUtils.isBlank(json)) {
					throw new IllegalArgumentException("Can't convert Offset to json : " + offset.toString());
				}
				try {
				...
				log.info("Successfully persist offset for topic:{},queueId:{}.", partitionedTopic.getTopic(), partitionedTopic.getIndex());
			}
		}
	}
	...
	private String buildPath(PartitionedTopic partitionedTopic) {
		return ZK_OFFSET_STORAGE_PREFIX_PATH + partitionedTopic.getTopic() + "/" + partitionedTopic.getIndex();
	}
}
